...
#| fig-cap: Mean values for evaluation of behavior towards waste disposal at the start and at the end of the survey
#| fig-width: 6
...
waste_disposal_behavior_tbl |> 
    gt() |> 
    tab_header(title = "Mean values for self-evaluation of behavior towards waste disposal at the start and at the end of the survey",
               subtitle = "Data from 19 participants") |>
  ...
  tab_options(
    ...
    heading.background.color = "#A2D1CC",
    column_labels.background.color = "#508F88"
  )
...
Based on the survey, the behavior of the participants towards waste disposal is changing depending on the situation. A time constraint seems to decrease the willingness to recycle properly. The behavior is also changing depending on the environment, as suggested by the comparison between recycling attitudes in nature or in the city, with a slightly better recycling in nature. Putting people in different fictional scenarios has an influence on the perception of their own recycling habits. This study shows that the environmental factors seems to influence how people recycle, but future studies are required to confirm the link between environmental factors and waste disposal behavior.
